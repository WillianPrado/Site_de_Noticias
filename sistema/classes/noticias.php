...
<?php 
	class produtos{
		...
		public function adicionarConteudo($dados){
		$c = new conectar();
		$conexao=$c->conexao();

		

		$sql = "UPDATE `noticias` SET `conteudo`=('$dados[1]') WHERE noticias.id = ('$dados[1]')";
		...
		public function inserirNoticia($dados){
			$c= new conectar();
			$conexao=$c->conexao();

			$data=date('Y-m-d');

			$sql="INSERT into noticias (titulo,
										id_imagem,
										id_estado,
										id_capital,
										id_tipoDeCaso,
